...
PeroxiCore — the Kotlin vanguard leading Mindustry modding into new frontiers. Raw power,
boundless reach, and imagination aimed at the stars.
...
`@ModConfig` removes the hand-written `mod.json` entirely — the KSP processor generates it
from
...
Adding `@ImportPeroxiCore` on a mod class emits a `peroxicore.json` marker. When the game
loads a
mod carrying that marker, `PXCPlatform` hands it an isolated class loader instead of the
calling
parent.
...
`ponder` provides a declarative scene DSL. The timeline is keyed by tick, and every
keyframe is a
...
Positions read as `(x at y)`, regions as `(range by range)` — see `PositionDsl` for the
full set.
The scene runs in a sandboxed copy of the world (`PonderGroups` swaps every `Groups` table
plus
...
- **TODO**: A written guide covering the module layout, the annotation pipeline and the
  Ponder DSL
